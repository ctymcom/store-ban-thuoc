    ...
    return (
            <div className="grid grid-cols-12 text-center text-16 sm:text-20 border-b-2 items-center overflow-hidden">
                <div className="col-span-8 sm:col-span-5 text-left flex items-center gap-2">
                    ...
                    onClick={()=> props.handleChangeItem(props.cartProduct.productId, "c", !props.cartProduct.active)}
                    >
                    ...
                    <div className={`w-11/12 flex gap-2 items-center sm:h-24  ${props.cartProduct.active?"":"opacity-50"}`}>
                        ...
                        <span className="block sm:hidden text-primary">
                            {NumberPipe(props.cartProduct.product.salePrice?
                                ...
                                props.cartProduct.product.basePrice, true)}</span></p>
                    </div>
                </div>
                <div className={`col-span-2 hidden sm:block ${props.cartProduct.active?"":"opacity-50"}`}>
                            {NumberPipe(props.cartProduct.product.salePrice?
                                props.cartProduct.product.salePrice:
                                props.cartProduct.product.basePrice, true)}
                </div>
                <div className={`col-span-4 sm:col-span-2 props.cartProduct.active?"":"opacity-50"}`} >
                    <div className="flex items-center justify-between w-full sm:w-5/6 mx-auto">
                        <i className="btn-default p-0 w-8 h-10 text-32 text-primary hover:text-primary-dark"
                            onClick={() => { props.handleChangeItem(props.cartProduct.productId, "sub", props.cartProduct.qty) }}
                            ><HiMinusCircle/></i>
                        <input className="w-full text-center" type="number"
                            value={props.cartProduct.qty < 10 ? '0' + props.cartProduct.qty : (props.cartProduct.qty).toString()}
                            onChange={(e) => { props.handleChangeItem(props.cartProduct.productId, "input", parseInt(e.target.value)) }} 
                            />
                        <i className="btn-default p-0 w-8 h-10 text-32 text-primary hover:text-primary-dark"
                            onClick={() => { props.handleChangeItem(props.cartProduct.productId, "add", props.cartProduct.qty) }}
                            ><HiPlusCircle/></i>
                    </div>
                </div>
                <div className={`col-span-2 hidden sm:block ${props.cartProduct.active?"":"opacity-50"}`}>
                    <div className="font-bold text-center">{NumberPipe(props.cartProduct.amount, true)}</div>
                </div>
                <div className={`col-span-1 grid-cols-5 hidden sm:block ${props.cartProduct.active?"":"opacity-50"}`}>    
                    <button className="col-span-1 btn-outline border-none text-20 text-danger hover:text-danger-dark" 
                    onClick={() => { props.handleDeleteCart(props.cartProduct.productId) }}
                    >Xóa</button>
                </div>
            </div>
    )
