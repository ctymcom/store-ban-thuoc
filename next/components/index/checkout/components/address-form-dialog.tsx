...
const AddressFormDialog = (props:PropsType) => {
  const {userAddress, handleChange, setUserAddress, provinces, districts, wards,listAddress } = useAddressContext();
    ...
    <Dialog width="420px" isOpen={props.isOpen} mobileMode={false}
                ...
                <Dialog.Body>
                  <div className="flex flex-wrap gap-4 py-4 text-16">
                    ...
                    <div className="w-full h-12 flex items-center gap-2 cursor-pointer" onClick={()=>{checkboxChange()}}>
                      <CheckBoxSquare checked={userAddress?userAddress.isDefault:true} /> Chọn làm địa chỉ mặc định
                      </div>
                    <button className="btn-primary w-full h-12 text-16" onClick={()=>{handleOnClick(userAddress)}}>Xác nhận</button>
                  </div>
                  <Dialog width="420px" isOpen={mess?true:false} mobileMode={false}
