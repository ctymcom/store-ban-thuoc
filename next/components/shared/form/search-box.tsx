...
import { IconArrowDown, IconInfor, IconSearch } from "../../../lib/svg"
...
export function SearchBox({ label, name, placeholder, required, value, style, options = [], noneOption, onChanged, onSearch, ...props }: SelectBoxProps) {
    ...
    return <>
        <div className="">
            <div className="relative" ref={wrapperRef}>
                ...
                <div className={(Show ? " border-primary-500 " : "  border-gray-300 ") + style + " transition border min-w-max w-full rounded-lg text-sm text-gray-400 relative flex items-center justify-between"}
                    onClick={() => setShow(true)}>

                    <input type="text" className=' rounded-lg p-4 w-full focus:outline-none' value={ValueInput}
                    ...
                    <div className={"w-5 h-5 transition absolute right-5 " + (Show && " text-secondary-400")}>
                        <IconSearch />
                    </div>
                </div>
                {
                            ...
                            <ul>
                                {
                                    (Options as any[]).map((item, index) => {
                                        return <li key={index} className={"p-4 hover:bg-gray-200 " + (SelectIndex == index && "bg-gray-200")}
                                            onClick={() => {
                                                setValueInput(item);
...
//         </div>}
//         <div className="">
//             <select onChange={onSelectChanged} defaultValue={value} required={required} name={name} className={style + ` form-select mt-1 w-full bg-white ring-gray-300 ring-1 p-4 text-sm text-gray-500 rounded-md focus:outline-none focus:ring-primary-500`}
//                 placeholder='placeholder'
//             >
//                 {noneOption && <option value={noneOption.value}>-- {noneOption.display || placeholder} --</option>}
//                 {(options as SelectBoxOption[]).map((o, index) =>
//                     <option key={o.value + index} className='bg-primary-500' value={o.value}>{o.display || o.value}</option>
//                 )}
//             </select>
//         </div>
//     </div>;
