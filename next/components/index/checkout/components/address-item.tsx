...
export function AddressItem(props:Proptype) {
    ...
    const {setAddressSelected, handleChange, addressSelected, setAddressEdit, setShowAddressFormDialog, addressEdit} = useAddressContext()
    useEffect(() => {
        setAddress(props.address);
      }, [props.address]);
    return (
        <div className={props.address.id===addressSelected.id?"border-b-2 text-16 sm:text-20 leading-7 py-4 text-gray-600 bg-primary-light":"border-b-2 text-16 sm:text-20 leading-7 py-4"} >
            ...
            <div className="flex items-center whitespace-nowrap text-gray-500 pt-2 w-11/12 mx-auto">
                ...
                <button className="ml-auto btn-default text-16 sm:text-20 px-0" 
                        onClick={()=>{setAddressEdit(address);setShowAddressFormDialog(true)}}><i className="p-1"><HiOutlinePencilAlt/></i>Chỉnh sửa</button>
