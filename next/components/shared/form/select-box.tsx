...
export function SelectBox({ label, name, placeholder, required, value, style, options = [], noneOption, onChanged }: SelectBoxProps) {
    ...
    return <div className="flex flex-col mb-3">
        ...
        <select onChange={onSelectChanged} defaultValue={value} required={required} name={name} className={style + `  border-solid  w-full h-full bg-white border-gray-300 border p-4 text-sm text-gray-500 rounded-md focus:outline-none focus:ring-secondary-500 focus:border-secondary-400 `}>
