        ...
        <>
          <div className={`${cellClass} flex col-span-5`} key={index}>
            ...
            <div className="pl-3">
              ...
              <div
                className="flex md:hidden flex-grow font-normal text-13 sm:text-16"
                key={index + "price"}
              >
                Giá: <span className="font-semibold ml-1">{NumberPipe(product.price, true)}</span>
              </div>
              <div
                className="flex md:hidden flex-grow font-normal text-13 sm:text-16"
                key={index + "qty"}
              >
                Số lượng: <span className="font-semibold ml-1">{5}</span>
              </div>
              <div
                className="flex md:hidden flex-grow font-normal text-13 sm:text-16"
                key={index + "product"}
              >
                Giá tiền:{" "}
                <span className="font-semibold ml-1">{NumberPipe(product.price, true)}</span>
              </div>
            </div>
          </div>
          <div className={`${cellClass} hidden justify-center col-span-1`} key={index + "price"}>
            {NumberPipe(product.price, true)}
          </div>
          <div className={`${cellClass} hidden justify-center col-span-1`} key={index + "qty"}>
            {5}
          </div>
          <div
            className={`${cellClass} hidden justify-end col-span-1 font-bold`}
            key={index + "product"}
          >
            {NumberPipe(product.price, true)}
          </div>
