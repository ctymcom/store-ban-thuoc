...
export function Select(props: PropsType) {
  return <div className={`relative flex items-center group ${props.className}`}>
    <select disabled={props.disabled?props.disabled:false} className={`border border-gray-400 rounded pl-4 pr-8 py-2 appearance-none hover:border-primary focus:border-primary-dark focus:outline-none ${props.className}`} 
      value={props.value}
