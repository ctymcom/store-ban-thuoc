...
export function HomepageSettings({
  ...
  return <>
    {
      settings.map((setting, index) => <div key={setting.id} className="mb-6">        
        ...
        {
          (setting.key == "BANNER_1" || setting.key == "BANNER_2" || setting.key == "BANNER_3") && <>
            <SettingItemsValue
              items={setting.value.items}
        ...
        {
          (setting.key == "HOTLINE") && <>
            <Input value={setting.value.headerText} prefix="Hotline header" {...inputProps}
            onChange={val => onValueChanged('headerText', val, index)}/>
            <Input value={setting.value.footerText} prefix="Hotline footer" {...inputProps}
            onChange={val => onValueChanged('footerText', val, index)}/>
            <Input value={setting.value.phone} prefix="Số điện thoại" {...inputProps}
        ...
        }
        {
          setting.key == "FOOTER_INTRO" && <>
            <Input value={setting.value.link} prefix="Link footer" {...inputProps}
            onChange={val => onValueChanged('link', val, index)}/>
            <Input value={setting.value.content} prefix="Nội dung footer" {...inputProps}
        ...
        {
          setting.key == "SOCIAL" && <>
            <Input value={setting.value.facebook.link} prefix="Link Facebook" 
              inputClassName="rounded-none rounded-tr form-input" prefixClassName="rounded-tl w-36"
            onChange={val => onValueChanged('facebook', { ...setting.value.facebook, link: val }, index)}/>
            <div className="flex mb-2">
              ...
              <div className="form-input flex-grow rounded-none rounded-br">
                <Switch value={setting.value.facebook.visible} 
                  onChange={val => onValueChanged('facebook',  { ...setting.value.facebook, visible: val }, index)}/>
              </div>
            </div>
          
            <Input value={setting.value.youtube.link} prefix="Link Youtube" 
              inputClassName="rounded-none rounded-tr form-input" prefixClassName="rounded-tl w-36"
            onChange={val => onValueChanged('youtube', { ...setting.value.youtube, link: val }, index)}/>
            <div className="flex mb-2">
              <div className="form-input hover:border-gray-400 w-36 bg-gray-100 border-r-0 rounded-none rounded-bl">
                Hiện Youtube
              </div>
              <div className="form-input flex-grow rounded-none rounded-br">
                <Switch value={setting.value.youtube.visible} 
                  onChange={val => onValueChanged('youtube',  { ...setting.value.youtube, visible: val }, index)}/>
              </div>
            </div>

            <Input value={setting.value.zalo.link} prefix="Link Zalo" 
              inputClassName="rounded-none rounded-tr form-input" prefixClassName="rounded-tl w-36"
            onChange={val => onValueChanged('zalo', { ...setting.value.zalo, link: val }, index)}/>
            <div className="flex mb-2">
              <div className="form-input hover:border-gray-400 w-36 bg-gray-100 border-r-0 rounded-none rounded-bl">
                Hiện Zalo
              </div>
              <div className="form-input flex-grow rounded-none rounded-br">
                <Switch value={setting.value.zalo.visible} 
                  onChange={val => onValueChanged('zalo',  { ...setting.value.zalo, visible: val }, index)}/>
              </div>
            </div>
          </>
        }
