...
import { PaginationPrevBtnCustom } from './pagination-prev-btn-custom';
import { PaginationNextBtnCusTom } from './pagination-next-btn-custom';
import { PaginationFirstPageBtnCustom } from './pagination-firstpage-btn-custom';
import { PaginationLastPageBtnCustom } from "./pagination-lastpage-btn-custom";
...
export function PaginationPages({ pagination, onPageChanged = () => {}, numOfPage = 5 }: PaginationPages) {
    ...
    const activeClass = "text-white transition-colors duration-150 bg-primary border-primary focus:text-white ";
    ...
    return <>
        <div className="pagination__page flex mt-4 justify-between w-full">
            <span className="flex items-center col-span-3 mr-8">
                Hiển thị {from} - {to} của {pagination.total} sản phẩm
            </span>
                        ...
                        {times(showPageCount, _ => pageStartIndex + _ + pageEndIndex).map((index) =>
                            <li key={'page' + (index)} className="mx-1.5">
                                <button
                                    ...
                                    className={" px-2.5 py-0.5 border-2 border-gray-600 hover:border-primary " +
                                        (index == pagination.page - 1 && activeClass) + " rounded-full focus:outline-none focus:shadow-outline-purple hover:text-primary "}>
