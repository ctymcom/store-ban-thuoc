          ...
          <div className="flex flex-col md:flex-row justify-between items-center border-b-2 py-5 md:py-3" key={index}>
            <div className="w-full md:w-3/5 text-sm md:text-base">
              ...
              <p className="pt-1 md:pt-0">Thời gian giao hàng dự kiến:  
                <span className="ml-2">{moment(item.delivery_time).format("DD/MM")} đến {moment(item.intend_time).subtract(-5).format("DD/MM/YYYY")}</span>
              </p>
            ...
            <button className={`${backgroundBtn} text-white px-6 py-1.5 rounded-md whitespace-nowrap text-sm md:text-base mt-4 md:mt-0`}>{showLabelStatus}</button>
          </div>
  </>;
