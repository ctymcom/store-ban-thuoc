...
const partitionArray = (array: any[], size) =>
  array
    .slice()
    .sort((a, b) => (a.content > b.content ? 1 : 0))
        ...
        {feedbackGroups.map((group, index) => (
          ...
          >
            {group.map((feedback, index) => (
              <div
                className="flex flex-col items-center md:flex-row md:items-start p-5 border border-primary rounded"
                key={index}
              >
                ...
                <div className="pl-0 mt-2 text-center md:pl-4 lg:pl-8 md:mt-0 md:text-left">
                  ...
                  <div className="text-gray-700 mt-2 relative">
                    <i className="absolute left-0 top-0 text-primary">
                      <ImQuotesLeft />
                    </i>
                    <p className="px-6">{feedback.content}</p>
                    <i className="absolute right-0 bottom-0 text-primary">
                      <ImQuotesRight />
                    </i>
                  </div>
                </div>
