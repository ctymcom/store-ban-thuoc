...
export function SelectBox({ label, name, placeholder, required, value, style, options = [], noneOption, onChanged, ...props }: SelectBoxProps) {
    ...
    return <div className="flex flex-col mb-3 space-y-1">
        ...
        <select onChange={onSelectChanged} defaultValue={value} required={required} name={name} className={style + ` w-full h-full bg-white ring-gray-300 ring-1 p-4 text-sm text-gray-500 rounded-md focus:outline-none focus:ring-secondary-500`}>
