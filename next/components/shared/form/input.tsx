...
export function Input({ label, name, required, placeholder, inputType = "text",value, ...props } : FormFieldProps) {
    ...
    return <div className="flex flex-col mb-3">
        ...
        <input onChange={onChange} value={value} required={required} name={name} className="border-solid w-full bg-white border-gray-300 border p-2 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" 
        type={inputType} 
