...
export function Checkbox({ label, name, required, onChanged, checked}: CheckBoxProps) {
    const onChange = (e) => {
        if (onChanged) onChanged(e.target.value);
    }
    return <div className="mb-3">
    <span className="mr-3"><input onChange={onChange} name={name} required={required} type="checkbox" checked={checked}/></span>
