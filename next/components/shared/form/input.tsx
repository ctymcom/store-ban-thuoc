...
export function Input({ label, name, required, placeholder, inputType = "text", icon = '', style, value, ...props }: FormFieldProps) {
    ...
    return <div className="flex flex-col mb-3 ">
        ...
        <input onChange={onChange}
            ...
            required={required}
            name={name}
            className={(icon != '' && "pl-10 ") + style + " border-solid  w-full h-full bg-white border-gray-300 border p-4 text-sm text-gray-500 rounded-md focus:outline-none focus:ring-secondary-500 focus:border-secondary-400"}
            type={inputType}
