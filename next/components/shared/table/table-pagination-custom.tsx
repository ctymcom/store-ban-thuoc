...
export function TablePaginationCustom({ pagination, onPageChanged, numOfPage = 5 }: TablePaginationProps) {
    ...
    const activeClass = "text-white duration-150 bg-primary-500";
    ...
    return <div
        className="flex justify-end px-5 py-3 text-xs font-semibold tracking-wide text-gray-500 uppercase border-t dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800"
    >
        <span className="flex mt-2 sm:mt-auto sm:justify-end">
            <nav aria-label="Table navigation">
                <ul className="inline-flex items-center">
                    {(pagination.page != 1) && <PaginationFirstPageButton onClick={() => onPageChanged(1)} />}
                    ...
                    {times(showPageCount, _ => pageStartIndex + _).map((index) => <li key={'page' + index}>
                        <button
                            ...
                            className={(index == pagination.page - 1 && activeClass) + " px-2 py-0.5 rounded-full focus:outline-none focus:shadow-outline-primary"}
                        >{index + 1}</button>
                    </li>)}
                    {(pagination.page < pageCount) && <PaginationNextButton onClick={() => onPageChanged(pagination.page + 1)} />}
                    {(pagination.page != pageCount) && <PaginationLastPageButton onClick={() => onPageChanged(pageCount)} />}
                </ul>
            </nav>
        </span>
    </div>
